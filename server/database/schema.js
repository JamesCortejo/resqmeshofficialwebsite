async function initializeSchema(db) {
  await db.exec(`
    ...
    CREATE TABLE IF NOT EXISTS device_offline_assignment_actions (
      id BIGINT GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
      action_id UUID NOT NULL UNIQUE,
      sync_device_id INTEGER REFERENCES sync_devices(id) ON DELETE SET NULL,
      source_node_id TEXT NOT NULL,
      deployment_id INTEGER NOT NULL REFERENCES distress_deployments(id),
      origin_node_id TEXT NOT NULL,
      origin_distress_id INTEGER NOT NULL,
      rescuer_code TEXT NOT NULL,
      occurred_at TIMESTAMPTZ NOT NULL,
      result TEXT NOT NULL CHECK (result IN ('accepted', 'already_applied', 'conflict', 'rejected')),
      reason TEXT,
      created_at TIMESTAMPTZ NOT NULL DEFAULT CURRENT_TIMESTAMP,
      updated_at TIMESTAMPTZ NOT NULL DEFAULT CURRENT_TIMESTAMP
    );
    ...
    CREATE INDEX IF NOT EXISTS idx_admin_action_audit_admin_user ON admin_action_audit_logs (admin_user_id, created_at DESC);
    CREATE INDEX IF NOT EXISTS idx_admin_action_audit_action ON admin_action_audit_logs (action, created_at DESC);
    ...
    CREATE INDEX IF NOT EXISTS idx_device_offline_actions_deployment
      ON device_offline_assignment_actions (deployment_id, created_at DESC);
    CREATE INDEX IF NOT EXISTS idx_device_offline_actions_device
      ON device_offline_assignment_actions (sync_device_id, created_at DESC);
  `);
}
